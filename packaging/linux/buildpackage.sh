...
install -Dm 0755 temp.desktop "${BUILTDIR}/usr/share/applications/opensa.desktop"
install -m 0755 temp.desktop "${BUILTDIR}/opensa.desktop"

sed "s/{MODID}/${MOD_ID}/g" include/mod-mimeinfo.xml.in | sed "s/{TAG}/${TAG}/g" > temp.xml
install -Dm 0755 temp.xml "${BUILTDIR}/usr/share/mime/packages/opensa.xml"

if [ -f "${PACKAGING_DIR}/mod_scalable.svg" ]; then
  install -Dm644 "${PACKAGING_DIR}/mod_scalable.svg" "${BUILTDIR}/usr/share/icons/hicolor/scalable/apps/opensa.svg"
fi

for i in 16x16 32x32 48x48 64x64; do
  if [ -f "${ARTWORK_DIR}/icon_${i}.png" ]; then
    install -Dm644 "${ARTWORK_DIR}/icon_${i}.png" "${BUILTDIR}/usr/share/icons/hicolor/${i}/apps/opensa.png"
    install -m644 "${ARTWORK_DIR}/icon_${i}.png" "${BUILTDIR}/opensa.png"
  fi
done
...
install -m 0755 openra-mod.temp "${BUILTDIR}/usr/bin/opensa"

sed "s/{MODID}/${MOD_ID}/g" include/mod-server.in  > openra-mod-server.temp
install -m 0755 openra-mod-server.temp "${BUILTDIR}/usr/bin/opensa-server"

sed "s/{MODID}/${MOD_ID}/g" include/mod-utility.in  > openra-mod-utility.temp
install -m 0755 openra-mod-utility.temp "${BUILTDIR}/usr/bin/opensa-utility"
